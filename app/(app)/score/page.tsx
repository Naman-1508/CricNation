    ...
    <div className="space-y-3">
      ...
      <button
        ...
        className={`w-full py-3 rounded-xl border text-sm font-medium transition-colors ${
          custom
            ? "border-[#E8390E] text-[#E8390E] bg-[#E8390E]/5"
      ...
      {custom && (
        <div className="flex items-center gap-3 bg-[#F2EFE9] rounded-xl px-4 py-3">
          <button type="button" onClick={() => onChange(Math.max(1, value - 1))}
            className="w-10 h-10 rounded-full border border-[rgba(107,74,42,0.2)] bg-white text-xl font-medium flex items-center justify-center hover:border-[#E8390E]">
            −
          </button>
          <input
            type="number"
...
function TeamPicker({ label, value, onSelect }: {
  label: string;
  value: { id: string; name: string; shortName: string; colorHex: string } | null;
  onSelect: (team: { id: string; name: string; shortName: string; colorHex: string }) => void;
}) {
  const [open, setOpen] = useState(false);
  const [query, setQuery] = useState("");
  const { data: myTeams } = trpc.team.getMyTeams.useQuery();
  const { data: searchResults } = trpc.team.search.useQuery(
    { query },
  ...
  return (
    <div>
      ...
      >
        {value ? (
          <>
            <div className="w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold text-white" style={{ backgroundColor: value.colorHex }}>
              {value.shortName}
            </div>
            <span className="font-medium text-sm text-[#1A1A1A]">{value.name}</span>
            <X className="w-4 h-4 text-[#8A8278] ml-auto" onClick={e => { e.stopPropagation(); onSelect(null as any); }} />
          </>
        ) : (
          <>
            <Search className="w-4 h-4 text-[#8A8278]" />
            <span className="text-sm text-[#8A8278]">Search or select team</span>
            <ChevronRight className="w-4 h-4 text-[#8A8278] ml-auto" />
          </>
        )}
      </button>
        ...
        {open && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 0.4 }} exit={{ opacity: 0 }}
              className="fixed inset-0 bg-black z-40" onClick={() => setOpen(false)} />
              ...
              <div className="overflow-y-auto flex-1 p-4 space-y-2">
                {!teams || teams.length === 0 ? (
                  <div className="py-8 text-center">
                    ...
                    <button onClick={() => { setOpen(false); }} className="text-[#E8390E] text-sm font-medium flex items-center gap-1 mx-auto">
                  ...
                  <button key={t.id} type="button"
                    ...
                    <div className="w-10 h-10 rounded-xl flex items-center justify-center font-bold text-white text-xs" style={{ backgroundColor: t.colorHex }}>
                      {t.shortName}
                    </div>
                    <div className="flex-1">
                      <p className="font-medium text-sm text-[#1A1A1A]">{t.name}</p>
                      <p className="text-xs text-[#8A8278]">{t.members?.length ?? t.memberCount ?? 0} members</p>
                    </div>
      ...
      <div className="bg-white border-b border-[rgba(107,74,42,0.13)] px-4 pt-12 pb-4 flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-[#8A8278]">
            {step === 1 ? "Select Teams" : step === 2 ? "Match Format" : "Toss & Confirm"}
          </p>
        </div>
      </div>
          ...
          {step === 3 && (
            <motion.div key="s3" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3 mb-5">
                  {([["home", homeTeam], ["away", awayTeam]] as const).map(([side, team]) => (
                    <button key={side} type="button" onClick={() => setTossWinner(side)}
                      ...
                      <div className="w-10 h-10 rounded-xl flex items-center justify-center font-bold text-white text-xs" style={{ backgroundColor: team?.colorHex ?? "#ccc" }}>
                        {team?.shortName ?? "?"}
                      </div>
              ...
              {tossWinner && tossDecision && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                  className="bg-[#1A1A1A] rounded-2xl p-5 text-white">
                  <p className="text-xs text-white/50 mb-3 uppercase tracking-wide">Match Summary</p>
                  <div className="flex items-center justify-between mb-4">
                    <div className="text-center">
                      <div className="w-12 h-12 rounded-xl mx-auto mb-1 flex items-center justify-center font-bold text-xs" style={{ backgroundColor: homeTeam?.colorHex }}>
                        {homeTeam?.shortName}
                      </div>
                    ...
                    <div className="text-center">
                      <div className="w-12 h-12 rounded-xl mx-auto mb-1 flex items-center justify-center font-bold text-xs" style={{ backgroundColor: awayTeam?.colorHex }}>
                        {awayTeam?.shortName}
                      </div>
                  ...
                  <div className="bg-white/10 rounded-xl px-4 py-2.5 text-center">
                    <p className="text-sm">
                      <span className="font-bold">{battingFirst?.name}</span>
                      <span className="text-white/60"> will bat first</span>
                    </p>
